...
BEGIN
  ...
  WHERE company_id IS NOT DISTINCT FROM q.company_id
  LIMIT 1;
  IF v_pct IS NULL THEN
    SELECT COALESCE(default_deposit_percentage, 50) INTO v_pct FROM public.company_settings LIMIT 1;
  END IF;
...
-- Accepting a quote now produces a DEPOSIT invoice, not a full-value one.
...
BEGIN
  IF OLD.status IS DISTINCT FROM 'accepted' AND NEW.status = 'accepted' THEN
    PERFORM public.create_deposit_invoice_for_quote(NEW.id);
  END IF;
  RETURN NEW;
END;
$$;

CREATE OR REPLACE FUNCTION public.accept_quote_by_token(p_token uuid, p_accepted_by text, p_signature jsonb DEFAULT NULL::jsonb)
RETURNS boolean
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
...
BEGIN
  ...
  -- Deposit invoice must exist immediately after acceptance.
  PERFORM public.create_deposit_invoice_for_quote(v_quote_id);
