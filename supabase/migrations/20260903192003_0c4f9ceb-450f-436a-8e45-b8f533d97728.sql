...
CREATE OR REPLACE FUNCTION public.get_my_assigned_jobs(p_profile_id uuid)
 RETURNS TABLE(assignment_id uuid, assignment_status text, job_id uuid, job_title text, job_description text, job_address text, job_status text, job_priority text, job_scheduled_for timestamp with time zone, customer_name text, customer_phone text, assignment_notes text, created_at timestamp with time zone, job_type text, deposit_invoice_id uuid, deposit_invoice_status text, deposit_invoice_paid_date timestamp with time zone, deposit_invoice_grand_total numeric, deposit_invoice_amount_paid numeric, deposit_invoice_remaining numeric)
 LANGUAGE plpgsql
 SECURITY DEFINER
